...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "photo_uuid",
            sa.String(36),
            sa.ForeignKey("inat_filtered_observations.photo_uuid"),
            primary_key=True,
        ),
        ...
        sa.Column(
            "proposed_bbox", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("conf", sa.Float(), nullable=True),
        sa.Column("annotation", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    )
    op.create_index(
        "ix_inat_obj_detection_dataset_train", "inat_obj_detection_dataset", ["train"]
    )
    op.create_index(
        "ix_inat_obj_detection_dataset_uiqm", "inat_obj_detection_dataset", ["uiqm"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_inat_obj_detection_dataset_uiqm", table_name="inat_obj_detection_dataset"
    )
    op.drop_index(
        "ix_inat_obj_detection_dataset_train", table_name="inat_obj_detection_dataset"
    )
