...
class InatBoundingBox:
    ...
    def __init__(
        ...
        bbox_folder_path: Path = Path(__file__).parents[1]
        / "data/classification_bboxs",
    ):
    ...
    def write_to_db(self, max_params: int = 65535):
        ...
        for row in data:
            row["proposed_bbox"] = (
                json.loads(row["proposed_bbox"]) if row["proposed_bbox"] else None
            )
            row["annotation"] = (
                json.loads(row["annotation"]) if row["annotation"] else None
            )
        ...
        with self.session_factory() as session:
            for i in range(0, len(data), chunk_size):
                ...
                logger.info(
                    "Upserted chunk %d/%d", min(i + chunk_size, len(data)), len(data)
                )
            session.commit()
        ...
        out_data = [
            {
                ...
                "annotation": json.dumps(
                    [{**self.annotation_normalization(row[1]), "class_id": 0}]
                ),
                "conf": row[1].get("conf"),
            }
            if row[1]
    ...
    def log_forced_negatives(self, photo_uuids: list[str]):
        ...
        """
        negative_annotation = json.dumps(
            [
                {
                    "class_id": 1,
                    "norm_center_x": None,
        ...
        with self.session_factory() as session:
            if self.mode == "detection":
                rows = session.execute(
                    ...
                    .join(
                        InatTaxa, InatFilteredObservations.taxon_id == InatTaxa.taxon_id
                    )
                ...
                return [
                    {
                        ...
                        "filename": r.filename,
                        "is_coral": _CORAL_TAXON in (r.ancestry or ""),
                    }
                    for r in rows
                ]
            else:
                rows = session.execute(
                    select(
                        self._target_model.photo_uuid, self._target_model.filename
                    ).where(self._target_model.filename.in_(file_list))
                ).all()
                return [
    ...
    def run_bbox_proposals(self):
        ...
        logger.info(
            "Found %d images in %s",
            len(filename_list),
            self.data_path / self.img_folder,
        )
        ...
        if self.mode == "detection":
            ...
            logger.info(
                "%d coral negatives (forced), %d fish images (inference)",
                len(coral),
                len(fish),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="MODELS",
        help=f"Model to use for bounding box inference. Choices: {', '.join(_MODELS)}.",
    )
    ...
    ibb = InatBoundingBox(mode=args.mode, conf=args.conf, model_path=args.model)
    if args.wal_only:
        ibb.write_to_db()
    else:
