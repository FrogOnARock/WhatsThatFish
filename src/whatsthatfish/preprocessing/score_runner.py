...
class ScoringProgressTracker:
    ...
    def __init__(
        ...
        self._source = source
        logger.debug(
            f"[{source}] ScoringProgressTracker initialised — WAL: {self._wal_path}, compact_every={compact_every}"
        )
    ...
    def load(self) -> set[str]:
        ...
        logger.info(
            f"[{self._source}] Loading progress from DB table '{self._dest_table.__tablename__}'"
        )
        with self._session_factory() as session:
            rows = (
                session.execute(select(self._dest_table.__table__.c[self._pk]))
                .scalars()
                .all()
            )
            ...
            if self._completed:
                logger.info(
                    f"[{self._source}] Loaded {len(self._completed):,} completed entries from DB"
                )
            else:
                logger.info(
                    f"[{self._source}] No prior entries found in DB — starting fresh"
                )
        ...
        if self._wal_path.exists():
            ...
            logger.debug(
                f"[{self._source}] No WAL file found at {self._wal_path} — will create on first record"
            )

        self._wal_file = open(self._wal_path, "a", newline="")
        self._wal_writer = csv.DictWriter(
            self._wal_file, self._dest_table.__table__.columns.keys()
        )
        if self._wal_path.stat().st_size == 0:
            self._wal_writer.writeheader()

        logger.info(
            f"[{self._source}] Total completed (DB + WAL): {len(self._completed):,}"
        )
    ...
    def compact(self) -> None:
        ...
        if not self._wal_buffer:
            logger.debug(
                f"[{self._source}] compact() called but WAL buffer is empty — skipping"
            )
            return
        ...
        logger.info(
            f"[{self._source}] Compact complete — {n:,} entries flushed, WAL truncated, total scored: {len(self._completed):,}"
        )
    ...
    def close(self) -> None:
        ...
        if self._wal_file:
            self._wal_file.close()
        logger.info(
            f"[{self._source}] Tracker closed. Total scored this session: {len(self._completed):,}"
        )
...
class ContextRunner:
    ...
    def __init__(
        ...
        logger.debug(
            f"[ContextRunner/{dataset}] Initialised with concurrency={concurrency}"
        )
    ...
    async def _context_with_tracking(
        ...
        async with self._semaphore:
            ...
            if isinstance(result, ValueError):
                logger.warning(
                    f"[ContextRunner] Skipping {identifier} — scorer returned: {result}"
                )
                return
    ...
    def _select_all_uploads(self):
        ...
        with self._session_factory() as session:
            ...
            logger.info(
                f"[ContextRunner] Found {len(ids):,} uploaded iNat photos in SuccessfulUploads"
            )
            return set(ids)
    ...
    def _select_files(self, photo_ids) -> list[dict[str, Any]]:
        ...
        with self._session_factory() as session:
            ...
            logger.info(
                f"[ContextRunner] Retrieved {len(result):,} file rows from InatFilteredObservations"
            )
            return result
    ...
    async def run(self):
        ...
        logger.info(
            f"[ContextRunner/{self._dataset}] {len(files):,} uploaded | {len(ids):,} already done | {len(rows):,} to score"
        )
        ...
        logger.info(
            f"[ContextRunner/{self._dataset}] {len(rows):,} rows → {total_batches:,} batches of {_BATCH_SIZE:,}"
        )
        ...
        try:
            for batch_idx, start in enumerate(range(0, len(rows), _BATCH_SIZE), 1):
                ...
                logger.info(
                    f"[ContextRunner/{self._dataset}] Batch {batch_idx}/{total_batches} — {len(batch):,} rows"
                )
                ...
                async with GCSAsyncStorage(
                    ...
                    context = (
                        [self._context_with_tracking(row, gcs_storage) for row in batch]
                    ...
                    if errors:
                        logger.warning(
                            f"[ContextRunner/{self._dataset}] Batch {batch_idx}: {len(errors):,}/{len(results):,} tasks failed — first: {type(errors[0]).__name__}: {errors[0]}"
                        )

                self._progress_tracker.compact()
                logger.info(
                    f"[ContextRunner/{self._dataset}] Batch {batch_idx}/{total_batches} complete — compacted"
                )

            logger.info(
                f"[ContextRunner/{self._dataset}] All context scoring batches complete"
            )
...
class ScoreRunner:
    ...
    def __init__(
        ...
        logger.debug(
            f"[ScoreRunner/{dataset}] Initialised with concurrency={concurrency}"
        )
    ...
    def _get_blob_details(self, row) -> tuple[str, str]:
        ...
        if self._dataset == "inat":
            ...
            raise ValueError("Dataset must be defined.")
    ...
    def _select_files(self, files) -> list[dict[str, Any]]:
        ...
        with self._session_factory() as session:
            ...
            logger.info(
                f"[ScoreRunner/inat] Retrieved {len(result):,} file rows from InatFilteredObservations"
            )
            return result
    ...
    def _select_all_uploads(self, dataset: str) -> set[str]:
        ...
        with self._session_factory() as session:
            if dataset == "inat":
                logger.debug(
                    "[ScoreRunner/inat] Querying InatClipContext for underwater images (is_underwater == 1)"
                )
                ids = (
                    session.execute(
                        select(InatClipContext.photo_uuid).where(
                            InatClipContext.is_underwater == 1
                        )
                    )
                    .scalars()
                    .all()
                ...
                logger.debug(
                    f"[ScoreRunner/{dataset}] Querying SuccessfulUploads for source='{dataset}'"
                )
                ids = (
                    session.execute(
                        select(SuccessfulUploads.identifier).where(
                            SuccessfulUploads.source == dataset
                        )
                    )
                    .scalars()
                    .all()
            ...
            logger.info(
                f"[ScoreRunner/{dataset}] Found {len(ids):,} candidates to score"
            )
            return set(ids)
    ...
    async def run(self):
        ...
        if self._dataset == "inat":
            ...
            rows = [r for r in rows if r.get("photo_uuid") not in ids]
            logger.info(
                f"[ScoreRunner/inat] {len(files):,} uploaded | {len(ids):,} already scored | {len(rows):,} to score"
            )
        else:
            files_to_process = files - ids
            rows = [{"filename": file} for file in files_to_process]
            logger.info(
                f"[ScoreRunner/lila] {len(files):,} uploaded | {len(ids):,} already scored | {len(rows):,} to score"
            )
        ...
        total_batches = (len(rows) + _BATCH_SIZE - 1) // _BATCH_SIZE
        logger.info(
            f"[ScoreRunner/{self._dataset}] {len(rows):,} rows → {total_batches:,} batches of {_BATCH_SIZE:,}"
        )
        ...
        try:
            for batch_idx, start in enumerate(range(0, len(rows), _BATCH_SIZE), 1):
                batch = rows[start : start + _BATCH_SIZE]
                logger.info(
                    f"[ScoreRunner/{self._dataset}] Batch {batch_idx}/{total_batches} — {len(batch):,} rows"
                )
                ...
                async with GCSAsyncStorage(
                    ...
                    scoring = [
                        self._scoring_with_tracking(row, gcs_storage) for row in batch
                    ]
                    results = await asyncio.gather(*scoring, return_exceptions=True)
                    errors = [r for r in results if isinstance(r, BaseException)]
                    if errors:
                        logger.warning(
                            f"[ScoreRunner/{self._dataset}] Batch {batch_idx}: {len(errors):,}/{len(results):,} tasks failed — first: {type(errors[0]).__name__}: {errors[0]}"
                        )

                self._progress_tracker.compact()
                logger.info(
                    f"[ScoreRunner/{self._dataset}] Batch {batch_idx}/{total_batches} complete — compacted"
                )

            logger.info(
                f"[ScoreRunner/{self._dataset}] All UIQM scoring batches complete"
            )
