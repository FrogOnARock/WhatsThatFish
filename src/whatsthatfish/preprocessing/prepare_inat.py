...
from ..database.models import InatClipContext, InatFilteredObservations, InatImageQuality, InatTaxa, InatClassificationDataset, InatObjDetectionDataset
...
class InatPreparation:
    ...
    def assign_split(self, rows: list[dict], val_floor: int = 20) -> list[dict]:
        ...
        train=0 → training, train=1 → validation (matches ClassificationDataset query).
        """
        ...
        val_clusters = set(
            rng.choice(unique_clusters, size=round(len(unique_clusters) * 0.2), replace=False).tolist()
        )
        ...
        def _build_indexes(rows):
            ...
            for i, row in enumerate(rows):
                (train_idx if row["train"] == True else val_idx)[row["taxon_id"]].append(i)
            return train_idx, val_idx

        taxon_train_index, taxon_val_index = _build_indexes(rows)

        drop_taxon_ids = {
            taxon_id for taxon_id, indices in taxon_train_index.items()
            if len(indices) < 50
        ...
        for idx, val in index_val_deficient.items():
            ...
            if not indices:
                continue
            index_to_move.extend(rng.choice(indices, size=min(to_move, len(indices)), replace=False).tolist())

        for idx in index_to_move:
            rows[idx]["train"] = True

        val_count = sum(1 for r in rows if r["train"] == False)
        logger.info(f"Split complete — train: {len(rows) - val_count}, val: {val_count}")
        return rows
    ...
    def load(self, rows) -> bool:
        ...
        od_rows = [
            {"photo_uuid": r["photo_uuid"], "filename": r["filename"], "uiqm": r["uiqm"], "train": r["train"]}
            for r in rows
        ]

        logger.info(
            "load: %d total rows → %d classification, %d OD (includes coral)",
            len(rows), len(classification_rows), len(od_rows),
        )
        ...
        with self.session() as session:
            ...
            session.execute(insert(InatClassificationDataset).values(classification_rows))
