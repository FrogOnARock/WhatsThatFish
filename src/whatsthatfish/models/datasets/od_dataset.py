...
class ObjectDetectionDataset(Dataset):
    ...
    def __init__(self, dataset: str, split: str = "train", transforms=None, max_samples: int = None, local_base_dir: str = None):
        self.session = get_session_factory()
        self.session_factory = self.session()
        self.split = True if split == "train" else False
        self.transform = transforms
        self.max_samples = max_samples if max_samples else 100000 if dataset in ["lc1", "lc2"] else None
        self.local_base_dir = Path(local_base_dir) if local_base_dir else (Path(__file__).parents[2] / "loaders/inat_od_images") \
            if dataset in ["lc1", "lc2"]\
            else (Path(__file__).parents[2] / "loaders/od_images")
        ...
        logger.info("Loaded %d records for dataset=%s split=%s", len(self.data), dataset, split)
    ...
    def labels(self):
        ...
        return [
            {
                "bboxes": np.array([[0, 0, 0, 0]]) if not record.annotation else np.array([
                    [ann["norm_center_x"], ann["norm_center_y"], ann["norm_width"], ann["norm_height"]]
                    if ann["class_id"] == 0
                    else [0, 0, 0, 0]
                    for ann in record.annotation
                ]),
                "cls": np.array([1]) if not record.annotation else np.array([ann["class_id"] for ann in record.annotation])
            }
    ...
    def __getitem__(self, idx):
        ...
        labels = [
            [ann["class_id"], ann["norm_center_x"], ann["norm_center_y"], ann["norm_width"], ann["norm_height"]]
