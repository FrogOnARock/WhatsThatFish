...
from ..retry import db_retry, transfer_retry
...
class ClipModel:
    ...
    async def _inference(self, row: dict[str, Any], text, gcs_storage):
        async with self._semaphore:
            ...
            tensor = self.preprocess(Image.open(BytesIO(blob))).unsqueeze(0).to(self.device)
            ...
            logger.debug(f"[ClipModel] {identifier} → pred={pred} scores={scores.tolist()}")
    ...
    async def run(self):
        ...
        logger.info(f"[ClipModel] {len(files):,} uploaded | {len(ids):,} already done | {len(rows):,} to score")
        ...
        logger.info(f"[ClipModel] {len(rows):,} rows → {total_batches:,} batches of {_BATCH_SIZE:,}")

        try:
            for batch_idx, start in enumerate(range(0, len(rows), _BATCH_SIZE), 1):
                batch = rows[start:start + _BATCH_SIZE]
                logger.info(f"[ClipModel] Batch {batch_idx}/{total_batches} — {len(batch):,} rows")
                ...
                async with GCSAsyncStorage(service_file=os.environ.get("GCS_SECRET")) as gcs_storage:
                    ...
                    if errors:
                        logger.warning(f"[ClipModel] Batch {batch_idx}: {len(errors):,}/{len(results):,} failed — first: {type(errors[0]).__name__}: {errors[0]}")

                self._progress_tracker.compact()
                logger.info(f"[ClipModel] Batch {batch_idx}/{total_batches} complete — compacted")
