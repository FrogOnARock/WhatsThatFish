...
import { SpeciesCard, GhostCard } from "../components/history/SpeciesCard";
import DetailPanel from "../components/history/DetailPanel";
import { getFieldLog } from "../api/client";
import type { FieldLog, LogSpecies } from "../api/types";
...
const lastSeen = (sp: LogSpecies): string =>
  sp.sightings.map((s) => s.date).sort().pop() ?? "";

const SORT_COMPARATORS: Record<string, (a: LogSpecies, b: LogSpecies) => number> = {
...
function uniqueSites(species: LogSpecies[]): number {
...
export default function HistoryPage() {
  const [log, setLog] = useState<FieldLog | null>(null);
