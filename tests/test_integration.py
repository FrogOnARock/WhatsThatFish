...
def _build_pt_pipeline(
    session_factory, tracker_dir: Path, compact_every: int
) -> PhotoTransferPipeline:
...
class TestFilteredObservationsInsert:
    ...
    def test_observations_insert_with_fk(self, session_factory, fixtures_dir):
        ...
        with session_factory() as session:
            ...
            row = session.execute(
                select(InatFilteredObservations).where(
                    InatFilteredObservations.photo_id == 10006
                )
            ).scalar_one()
    ...
    def test_casual_and_non_fish_filtered_out(self, session_factory, fixtures_dir):
        ...
        with session_factory() as session:
            ...
            casual = session.execute(
                select(InatFilteredObservations).where(
                    InatFilteredObservations.photo_id == 10008
                )
            ).scalar_one_or_none()
            ...
            insect = session.execute(
                select(InatFilteredObservations).where(
                    InatFilteredObservations.photo_id == 10009
                )
            ).scalar_one_or_none()
...
class TestWALCrashRecovery:
    def test_wal_replay_against_postgres(
        self, session_factory, fixtures_dir, tracker_dir: Path
    ):
        ...
        pt_pipeline = _build_pt_pipeline(
            session_factory, tracker_dir, compact_every=1000
        )
        ...
        pt_pipeline_1 = _build_pt_pipeline(
            session_factory, tracker_dir, compact_every=1000
        )
...
class TestLilaFKConstraint:
    ...
    def test_images_then_annotations_succeeds(self, session_factory, fixtures_dir):
        ...
        with session_factory() as session:
            ...
            frame_100_anns = (
                session.execute(
                    select(LilaAnnotations).where(
                        LilaAnnotations.image_id == "salmon_cv/frame_00100.jpg"
                    )
                )
                .scalars()
                .all()
    ...
    def test_negative_frames_have_no_annotations(self, session_factory, fixtures_dir):
        ...
        with session_factory() as session:
            ...
            neg_frame_1 = session.execute(
                select(LilaCollectedImages).where(
                    LilaCollectedImages.id == "salmon_cv/frame_00400.jpg"
                )
            ).scalar_one()
            ...
            neg_frame_2 = session.execute(
                select(LilaCollectedImages).where(
                    LilaCollectedImages.id == "brackish/seq01_000300.jpg"
                )
            ).scalar_one()
