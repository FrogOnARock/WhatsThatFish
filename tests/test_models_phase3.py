...
TAXA = [
    {"taxon_id": 1001, "zero_index": 0, "species": "Amphiprion ocellaris",
     "genus": "Amphiprion", "family": "Pomacentridae"},
    {"taxon_id": 2001, "zero_index": 1, "species": "Thalassoma lunare",
     "genus": "Thalassoma", "family": "Labridae"},
]
...
class TestConstraints:
    ...
    def test_observation_label_status_server_default(self, session_factory, seed_taxa, make_user):
        seed_taxa(TAXA)
        user = make_user()
        with session_factory() as s:
            ...
            s.flush()
            obs = Observation(
                dive_id=dive.id, user_id=user.id,
                predicted_taxon_id=1001, corrected_taxon_id=1001,
            )
            s.add(obs)
            s.commit()
            s.refresh(obs)
    ...
    def test_two_taxon_fks_resolve_independently(self, session_factory, seed_taxa, make_user):
        seed_taxa(TAXA)
        user = make_user()
        with session_factory() as s:
            dive = Dive(user_id=user.id)
            ...
            obs = Observation(
                dive_id=dive.id, user_id=user.id,
                predicted_taxon_id=1001, corrected_taxon_id=2001,
            )
            s.add(obs)
            s.commit()
    ...
    def test_corrected_taxon_id_is_not_nullable(self, session_factory, seed_taxa, make_user):
        seed_taxa(TAXA)
        user = make_user()
        with session_factory() as s:
            dive = Dive(user_id=user.id)
            s.add(dive)
            s.flush()
            s.add(Observation(dive_id=dive.id, user_id=user.id, predicted_taxon_id=1001))
            with pytest.raises(IntegrityError):
                s.commit()


class TestCascade:
    def test_delete_user_cascades_to_dives_obs_photos(self, session_factory, seed_taxa, make_user):
        seed_taxa(TAXA)
        user = make_user()
        with session_factory() as s:
            dive = Dive(user_id=user.id)
    ...
    def test_delete_dive_cascades_to_observations(self, session_factory, seed_taxa, make_user):
        seed_taxa(TAXA)
        user = make_user()
        with session_factory() as s:
            dive = Dive(user_id=user.id)
