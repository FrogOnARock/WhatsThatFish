...
class TestDives:
    ...
    def test_update_dive_changes_site_and_notes(self, obs_env):
        ...
        out = svc.update_dive(
            user, dive.id, DiveUpdate(site_name="New Site", notes="viz 30m")
        )
    ...
    def test_list_dives_enrichment_counts_and_dedupes_species(self, obs_env):
        ...
        for taxon in (1001, 1001, 2001):  # 3 obs, 2 distinct species
            svc.create_observation(
                user,
                ObservationCreate(
                    dive_id=dive.id,
                    predicted_species_index=_index_of(taxon),
                ),
            )
...
class TestObservationCreate:
    def test_predicted_only_sets_effective_to_predicted(self, obs_env):
        ...
        out = svc.create_observation(
            user, ObservationCreate(dive_id=dive.id, predicted_species_index=0)
        )
    ...
    def test_unknown_predicted_index_raises_validation(self, obs_env):
        ...
        with pytest.raises(ValidationException):
            svc.create_observation(
                user, ObservationCreate(dive_id=dive.id, predicted_species_index=999)
            )
    ...
    def test_dive_not_found_raises(self, obs_env):
        ...
        with pytest.raises(ResourceNotFoundException):
            svc.create_observation(
                user, ObservationCreate(dive_id=uuid.uuid4(), predicted_species_index=0)
            )
...
class TestObservationUpdate:
    def _obs(self, svc, user):
        dive = svc.create_dive(user, DiveCreate())
        return svc.create_observation(
            user,
            ObservationCreate(dive_id=dive.id, predicted_species_index=0, depth_m=5.0),
        )

    def test_relabel_and_depth(self, obs_env):
        svc, user, _ = obs_env
        obs = self._obs(svc, user)
        out = svc.update_observation(
            user,
            obs.id,
            ObservationUpdate(
...
class TestPhotosAndFieldLog:
    def test_add_photo_uploads_and_translates_index(self, obs_env):
        ...
        photo = svc.add_photo(
            user, obs.id, b"jpegbytes", predicted_species_index=2, confidence=0.8
        )
    ...
    def test_field_log_groups_by_effective_taxon(self, obs_env):
        ...
        for idx in (0, 0, 2):
            svc.create_observation(
                user, ObservationCreate(dive_id=dive.id, predicted_species_index=idx)
            )
...
class TestUserSettings:
    ...
    def test_update_sets_name_and_units(self, session_factory):
        with session_factory() as s:
            svc = UserService(s)
            user = svc.get_or_create(USER_A)
            prof = svc.update_settings(
                user,
                UserSettingsUpdate(preferred_name="Reef Diver", unit_system="imperial"),
            )
